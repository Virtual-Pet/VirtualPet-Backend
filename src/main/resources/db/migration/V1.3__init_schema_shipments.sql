-- =============================================================================
-- SCHEMA: shipments
--    Módulo propietario: com.virtualpet.shipments
...
INSERT INTO shipments.warehouses (name, city, state, zip_code)
...
CREATE TABLE shipments.shipments (
     ...
     CONSTRAINT fk_shipments_warehouse FOREIGN KEY (warehouse_id) REFERENCES shipments.warehouses(id) ON DELETE RESTRICT,
     CONSTRAINT chk_courier CHECK (courier IN ('OWN_DELIVERY', 'ANDREANI', 'OCA', 'CORREO_ARG')),
     CONSTRAINT chk_shipment_status CHECK (status IN ('CONFIRMED', 'PREPARED', 'IN_TRANSIT', 'DELIVERED', 'CANCELLED')),
     CONSTRAINT chk_shipment_attempts CHECK (attempts BETWEEN 0 AND 3),
     CONSTRAINT uq_shipment_order UNIQUE (order_id)
);

-- =============================================================================
-- TABLE: shipment_status (Auditoría de tiempos de envíos)
-- =============================================================================
CREATE TABLE shipments.shipment_status (
       id              UUID            DEFAULT gen_random_uuid(),
       ...
       CONSTRAINT fk_history_shipment FOREIGN KEY (shipment_id) REFERENCES shipments.shipments(id) ON DELETE CASCADE
);

-- =============================================================================
-- TABLE: shipment_attempts (Historial de reintentos)
-- =============================================================================
CREATE TABLE shipments.shipment_attempts (
     id                  UUID            DEFAULT gen_random_uuid(),
     shipment_id         UUID            NOT NULL,
     ...
     CONSTRAINT fk_attempts_shipment FOREIGN KEY (shipment_id) REFERENCES shipments.shipments(id) ON DELETE CASCADE,
...
-- =============================================================================
CREATE TABLE shipments.order_assignments (
     id              UUID            DEFAULT gen_random_uuid(),
     ...
     CONSTRAINT fk_assignment_warehouse FOREIGN KEY (warehouse_id) REFERENCES shipments.warehouses(id) ON DELETE RESTRICT,
...
-- ÍNDICES
-- =============================================================================
CREATE INDEX idx_shipments_order_id ON shipments.shipments (order_id);
CREATE INDEX idx_shipments_status ON shipments.shipments (status);
CREATE INDEX idx_shipments_warehouse ON shipments.shipments (warehouse_id);

CREATE INDEX idx_shipment_history_id ON shipments.shipment_status (shipment_id);

CREATE INDEX idx_shipment_attempts_id ON shipments.shipment_attempts (shipment_id);

CREATE INDEX idx_assignments_order_id ON shipments.order_assignments (order_id);
CREATE INDEX idx_assignments_operator ON shipments.order_assignments (operator_id);
CREATE INDEX idx_assignments_wh_status ON shipments.order_assignments (warehouse_id, status);
