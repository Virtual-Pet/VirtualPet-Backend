...
public class EmployeeOrchestrator {
  ...
  public void executeRegister(RegisterEmployeeRequest request) {
    ...
    String formatRole =
        request.role().toUpperCase().startsWith("ROLE_")
    ...
    UserEntity user =
        authService.createIdentity(
            request.email(), request.temporaryPassword(), UserRole.valueOf(formatRole), true);
                      ...
                      .orElseThrow(
                          () ->
                              new ApiException(
                                  ...
                                  "Error de integridad: No se encontró la identidad para el empleado "
                                      + profile.getLegajo()));
